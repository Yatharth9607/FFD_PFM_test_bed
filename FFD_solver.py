...
class CFD:
        ...
        for k in range(1, vel_iter+1):
            for i in range(1, N):
                for j in range(1, N+1):
                    ...
                    b = max(UL, 0) * self.U[t,i-1,j] + max(VB, 0) * self.U[t,i,j-1] - min(UR, 0) * self.U[t,i+1,j] - min(VT, 0) * self.U[t,i,j+1]
                    c = max(UL, 0) + max(VB, 0) - min(UR, 0) - min(VT, 0)

                    RUSum = self.U[t,i,j+1] / RUY[i,j] + self.U[t,i,j-1] / RUY[i,j-1] + self.U[t,i-1,j] / RUX[i-1,j] + self.U[t,i+1,j] / RUX[i,j]
                    RUSumCoeff = 1 / RUY[i,j] + 1 / RUY[i,j-1] + 1 / RUX[i-1,j] + 1 / RUX[i,j]

                    Num = self.U[t-1,i,j] + (dt/dx) * b + (dt/(rho*dx)) * RUSum
        ...
        for k in range(1, vel_iter + 1):
            for i in range(1, N + 1):
                for j in range(1, N):
                    ...
                    b = max(UL, 0) * self.V[t,i-1,j] + max(VB, 0) * self.V[t,i,j-1] - min(UR, 0) * self.V[t,i+1,j] - min(VT, 0) * self.V[t,i,j+1]
                    c = max(UL, 0) + max(VB, 0) - min(UR, 0) - min(VT, 0)

                    RVSum = self.V[t,i,j+1] / RVY[i,j] + self.V[t,i,j-1] / RVY[i,j-1] + self.V[t,i-1,j] / RVX[i-1,j] + self.V[t,i+1,j] / RVX[i,j]
                    RVSumCoeff = 1/RVY[i,j] + 1/RVY[i,j-1] + 1/RVX[i-1,j] + 1/RVX[i,j]

                    Force = -np.sign(self.V[t-1,i,j]) * ((LossCoeffY[i,j]*dt) / (2*dx)) * self.V[t-1,i,j]**2 + g * beta * (self.T[t,i,j] - T_ref) * dt

                    Num = self.V[t-1,i,j] + Force + (dt/dx) * b + (dt/(rho*dx)) * RVSum
    ...
    def project(self, N, t, FX, FY, dt, dx):
        ...
        for i in range(1, N+1):
            for j in range(1, N+1):
                div[i,j] = (1/dx) * (self.U[t-1,i,j] - self.U[t-1,i-1,j] + self.V[t-1,i,j] - self.V[t-1,i,j-1])
        ...
        for k in range(1, proj_iter+1):
            ...
            for i in range(1, N+1):
                for j in range(1, N+1):
                    ...
                    PFSum[i,j] = P[i+1, j] * FX[i, j] + P[i-1, j] * FX[i-1, j] + P[i, j+1] * FY[i, j] + P[i,j-1] * FY[i, j-1]
                    PFSumCoeff = FX[i, j] + FX[i-1, j] + FY[i, j] + FY[i, j-1]
                    self.P[i, j] = (1 - omega)*P[i, j] + omega*(PFSum[i,j] - (rho*dx**2/dt)*div[i, j])/PFSumCoeff
        
        # Compute new U velocities for interior boundaries

        for i in range(1, N):
            for j in range(1, N+1):
                self.U[t,i,j] = self.U[t-1,i,j] - (dt/(rho*dx)) * (self.P[i+1,j] - self.P[i,j])

        # Compute new V velocities for interior boundaries

        for i in range(1, N+1):
            for j in range(1, N):
                self.V[t,i,j] = self.V[t-1,i,j] - (dt/(rho*dx)) * (self.P[i,j+1] - self.P[i,j])
        ...
        for k in range(1, T_iter+1):
            for i in range(1, N+1):
                for j in range(1, N+1):
                    ...
                    b = max(UL,0)*self.T[t,i-1,j] + max(VB,0)*self.T[t,i,j-1] - min(UR,0)*self.T[t,i+1,j] - min(VT,0)*self.T[t,i,j+1]
                    c = max(UL,0) + max(VB,0) - min(UR,0) - min(VT,0)

                    RTSum = self.T[t,i,j+1]/RTY[i,j] + self.T[t,i,j-1]/RTY[i,j-1] + self.T[t,i-1,j]/RTX[i-1,j] + self.T[t,i+1,j]/RTX[i,j]
                    RTSumCoeff = 1/RTY[i,j] + 1/RTY[i,j-1] + 1/RTX[i-1,j] + 1/RTX[i,j]

                    Num = self.T[t-1,i,j] + (dt/(rho*Cp*dx**2))*q + (dt/dx)*b + (dt/(rho*Cp*dx))*RTSum
    ...
    def calcMassBalance(self, N, t):
        ...
        for i in range(1, N + 1):
            for j in range(1, N + 1):
                mass[i - 1, j - 1] = self.U[t, i, j] + self.V[t, i, j] - self.U[t, i - 1, j] - self.V[t, i, j - 1]
        return mass
